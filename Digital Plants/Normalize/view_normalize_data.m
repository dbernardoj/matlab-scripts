...
load('/Users/pma007/Dropbox/Vale/Digital Plants/Vetor-base/vsps_mami_absMom_N4_vetorAHVD_db3_14c.mat')
load('species');
%% PRE-PROCESS DATA
sp1 = vetor(1:59,1:128);
sp2 = vetor(60:122,1:128);
vetor = vetor(:,1:128);

%%
%%% Plot of the mean of species 1 and 2 (First Normalization)
Vsp1_std = std(sp1,0,1);
Vsp2_std = std(sp2,0,1);
h1 = figure;
plot(mean(sp1),'bo-','MarkerFaceColor',[.49 1 .63],'MarkerSize',10);
hold
errorbar(mean(sp1),Vsp1_std);
plot(mean(sp2),'r^-','MarkerFaceColor',[.29 .5 .23],'MarkerSize',10);
errorbar(mean(sp2),Vsp2_std,'r');
...
Tree_gdi = ClassificationTree.fit(vetor,especie,'SplitCriterion','gdi');
view(Tree_gdi, 'mode', 'graph');

Tree_twoing = ClassificationTree.fit(vetor,especie,'SplitCriterion','twoing');
view(Tree_twoing, 'mode', 'graph');

Tree_deviance = ClassificationTree.fit(vetor,especie,'SplitCriterion','deviance');
view(Tree_deviance, 'mode', 'graph');
%% SECOND STEP

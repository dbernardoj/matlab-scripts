function [ results,eval_results ] = compareFeatures_2( X,y,features1,features2 )
...
cfMat1 = crossval(f_conf_mat,X,y,'partition',cpLeaveoneout);
if nclasses ~= 2
...
cfMat2 = crossval(f_conf_mat,X(:,features1),y,'partition',cpLeaveoneout);
if nclasses ~= 2
...
cvErrLO03 = crossval('mcr',X(:,features2),y,'predfun',classf,...
    'partition',cpLeaveoneout);
cfMat3 = crossval(f_conf_mat,X(:,features2),y,'partition',cpLeaveoneout);
if nclasses ~= 2
    cfMat3 = reshape(sum(cfMat3),nclasses,nclasses);
else
    cfMat3 = reshape(sum(cfMat3),2,2);
end
...
N3 = sum(sum(cfMat3));
Acc3 = sum(diag(cfMat3)) / N3;
pe3 = 0;
SpecAcc3 = zeros(1,length(cfMat3));
for i = 1:length(cfMat3)
    pe3 = pe3 + (sum(cfMat3(:,i)) * sum(cfMat3(i,:)));
    SpecAcc3(i) = 2 * cfMat3(i,i) / (sum(cfMat3(:,i)) + sum(cfMat3(i,:)));
end
pe3 = pe3 / N3^2;
Kappa3 = (Acc3 - pe3) / (1 - pe3);
...
% Selected features 2
eval_results3 = cell(4,length(cfMat3)+2);
% LABELS
eval_results3{1,2} = 'Measure';
for i = 1:length(cfMat3)
    eval_results3{1,i+2} = sprintf('Class %d - %s',i,order{i});
    eval_results3{3,i+2} = SpecAcc3(i);
end
eval_results3{2,1} = 'Accuracy';
eval_results3{3,1} = 'Specific Accuracy';
eval_results3{4,1} = 'Kappa';
% RESULTS
eval_results3{2,2} = Acc3;
eval_results3{4,2} = Kappa3;

% Put in place
eval_results{1,1} = 'All features';
eval_results{2,1} = eval_results1;
eval_results{3,1} = cfMat1;
eval_results{1,2} = 'Selected features 1';
eval_results{2,2} = eval_results2;
eval_results{3,2} = cfMat2;
eval_results{1,3} = 'Selected features 2';
eval_results{2,3} = eval_results3;
eval_results{3,3} = cfMat3;
end
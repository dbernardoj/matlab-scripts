...
% Last Modified by GUIDE v2.5 16-May-2014 18:05:19
...
ss = get(0,'ScreenSize');
%mp = get(handles.main,'Position');
screenSize.width = ss(3);
screenSize.height = ss(4);
%mainPosition.x = mp(1);
%mainPosition.y = mp(2);
setappdata(handles.main,'screensize',screenSize);
%setappdata(handles.main,'mainposition',mainPosition);
    ...
    if ~isempty(nas)
        ...
        if ~isempty(fsas)
            ...
            elseif strcmp(fsas_str,'Sequential Forward Selection')
                % Set waiting flag in appdata
                setappdata(handles.main,'waiting',1)
                [answer,Cancelled] = sfs_dialog('fs_main', handles.main);
                if ~Cancelled
                    setappdata(handles.main,'waiting',0)
                    ax1 = handles.fs_plot1; % get axes fs_plot1
                    ax2 = handles.fs_plot2; % get axes fs_plot2
                    %ax1 = axes(handles.fs_plot1); % set the current axes to fs_plot1
                    %ax2 = axes(handles.fs_plot2); % set the current axes to fs_plot2
                    cla(handles.fs_plot1,'reset'); % clear specific axes
                    cla(handles.fs_plot2,'reset'); % clear specific axes
                    sel_feats = sfs_func(X,data.Y,answer.ClassAlg,...
                        answer.Crossval,answer.CrossvalInput,ax1,ax2);
                    set(handles.fs_table,'Data',sel_feats);
                else
                    setappdata(handles.main,'waiting',0)
                end
            elseif strcmp(fsas_str,'Anova')
                Title = 'Parameters input';
                ...
                if ~Cancelled
                    ...
                    cla(handles.fs_plot1,'reset'); % clear specific axes
                    cla(handles.fs_plot2,'reset'); % clear specific axes
                    sel_feats = Anova_fs(X,data.Y,Answer.Sig_level,ax);
                    set(handles.fs_table,'Data',sel_feats);
                end
            else
...
screensize = getappdata(handles.main,'screensize');
if ~isempty(screensize)
    width = screensize.width/2;
    height = screensize.height*0.9;
    left = (screensize.width-width)/2;
    bottom = screensize.height-height;
    set(handles.main,'Position',[left bottom width height])
end
